...
{  // Guardamos la operacion dentro de la pila
  ...
  for (auto i : line) {  // check line length some caracter not saving well
    if (i != ' ' && i != '\n' && i != '\r') {
  ...
  while (arrstr_.size() > count) {
    ...
    if (str.size() > 1) {  // comprobamos si es un operando
      for (size_t i = 0; i < lang_r.size(); i++) {  // Lo asignamos a la pila
      ...
      if (str[0] != '!' && str[0] != '*' && str[0] != '-' && str[0] != '+' &&
        ...
        n = atoi(&st);
                 ...
                 str[0] != '*') {   // si el operador tomas n operandos
        if (stacklang_.size() < 2)  // comprobamos que hayan 2 operandos
        {
          ...
          Lenguaje x;
          switch (str[0]) {  // realizamos la operacion
            case '+':
